...
class AAMSScheduler:
    ...
    def _time_cost_kmin(self, selected, k_min):
        times = {}
        for d in selected:
        ...
        round_time        = times[fast_devices[-1]]
    ...
    def _total_cost(self, selections, num_active=2):
        ...
        for j, selected in selections.items():
            if selected:
                ...
                rt, _, _, _ = self._time_cost_kmin(selected, k_min)
    ...
    def simulate_and_reallocate(self, selections, job_done, clients, num_active):
        ...
        for j, selected in selections.items():
            ...
            k_min = self.get_adaptive_kmin(j, num_active)
            rt, fast, stragglers, _ = self._time_cost_kmin(selected, k_min)
...
def main():
    ...
    print('=' * 70)
    print('AAMS — Adaptive Async Multi-Job Scheduling')
    print('=' * 70)
    ...
    with open('results/aams_log.json', 'w') as f:
        json.dump(log, f)
